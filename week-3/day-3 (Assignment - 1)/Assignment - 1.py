...
# MAGIC %md
# MAGIC **Data Frame Creation and Reading**
# MAGIC
...
df.filter(df.department=="IT").select("emp_id","emp_name","salary").show()
...
df.filter(df.city=="Hyderabad").select("emp_name","city").show()
...
df.select(df.emp_name.alias("name"),df.city.alias("location")).show()
...
df.select(df.emp_id.alias("id"),df.emp_name.alias("name"),df.salary.alias("income")).show()

# 9. Display salary as emp_salary and department as emp_dept
df.select(df.salary.alias("emp_salary"),df.department.alias("emp_dept")).show()
...
# MAGIC %md
# MAGIC **Filter commands using WHERE**
...
# MAGIC %md
# MAGIC **Commands with column name change**
...
df.withColumnRenamed("emp_id","employee_id").withColumnRenamed("salary","income").show()
...
# MAGIC %md
# MAGIC **Commands involving creation of columns**
...
df.withColumn("tax",df.salary*0.05).show()

# 4. Create updated_salary by adding 5000
df.withColumn("updated_salary",df.salary+5000).show()

# 5. Create salary_category column
df.withColumn("salary_category",when(df.salary>80000,"High").when(df.salary>50000,"Medium").otherwise("Low")).show()

# 6. Create age_group column
df.withColumn("age_group",when(df.age<25,"Young").otherwise("Adult")).show()

# 7. Create location column by combining city and department
df.withColumn("location",concat_ws("-",df.city,df.department)).show()

# 8. Create increment_salary column with 15% hike
df.withColumn("increment_salary",df.salary*1.15).show()

# 9. Create experience_status column based on joining year
df.withColumn("experience_status",when(year("joining_date")<2021,"Experienced").otherwise("New")).show()

# 10. Create name_length column
df.withColumn("name_length",length("emp_name")).show()

# 11. Create is_high_salary column
df.withColumn("is_high_salary",df.salary>70000).show()

# 12. Create joining_year column
df.withColumn("joining_year",year("joining_date")).show()

# 13. Create salary_after_tax column
df.withColumn("salary_after_tax",df.salary-(df.salary*0.05)).show()

# 14. Create department_code column
df.withColumn("department_code",substring("department",1,3)).show()
...
df.withColumn("age",df.age.cast("double")).show()

# 3. Convert joining_date to date datatype
df.withColumn("joining_date",to_date("joining_date")).show()

# 4. Convert emp_id to string datatype
df.withColumn("emp_id",df.emp_id.cast("string")).show()

# 5. Convert salary to integer datatype
df.withColumn("salary",df.salary.cast("int")).show()

# 6. Convert age to string datatype
df.withColumn("age",df.age.cast("string")).show()

# 7. Convert joining_date to timestamp datatype
df.withColumn("joining_date",to_timestamp("joining_date")).show()

# 8. Convert salary to float datatype
df.withColumn("salary",df.salary.cast("float")).show()

# 9. Convert emp_id to long datatype
df.withColumn("emp_id",df.emp_id.cast("long")).show()

# 10. Convert multiple columns into different datatypes
df.withColumn("age",df.age.cast("double")).withColumn("salary",df.salary.cast("float")).show()

# COMMAND ----------

# MAGIC %md
# MAGIC **Sort & Order by commands**
...
df.filter(df.department=="IT").orderBy(df.salary.desc()).show()
...
df.filter(df.salary>60000).limit(8).show()